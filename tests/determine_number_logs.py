...
RAW_ROOT = Path(
    "C:/Users/Ninzz/Programming/PLD/up-dsp-pld/PLD/CEB"
)
...
def load_number_mapping() -> dict[str, str]:
    ...
    if not isinstance(mapping, dict):
        ...
        print(
            "ERROR: numbers.json must contain "
            "a JSON object.",
            file=sys.stderr,
        )
...
def detect_numbers(
    ...
    for number in matches:
        ...
        if number in number_mapping:
            ...
            print(
                f"WARNING: number {number!r} "
                f"not found in numbers.json",
                file=sys.stderr,
            )
        ...
        for line_number, line in enumerate(
            ...
            if not detected_numbers:
                ...
                print(
                    ...
                    f"{log_file.name}:"
                    f"{line_number}: "
                    f"Random Digit entry contains "
                    f"no recognized number: "
                    f"{text!r}",
                    file=sys.stderr,
                )
...
def main():
    ...
    print(
        f"Speakers with results : "
        f"{speakers_processed}"
    )

    print(
        f"Speakers without logs : "
        f"{speakers_without_logs}"
    )
