...
Usage:
    python preprocessing\select_speakers.py \
        --manifest manifests\manifest_bonafide.csv \
        --fraction 0.5 \
        --seed 42 \
        --out manifests\mms_selected_speakers.csv
"""
...
def load_speaker_gender(manifest_path: Path) -> dict:
    ...
    with open(manifest_path, "r", encoding="utf-8", newline="") as f:
        ...
        if (
            ...
            print(
                "ERROR: manifest must have 'speaker_id' and 'speaker_gender'"
                " columns.",
                file=sys.stderr,
            )
            sys.exit(1)
...
def load_clip_stats(manifest_path: Path) -> dict:
    ...
    with open(manifest_path, "r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            try:
                stats[spk]["duration_sec"] += float(
                    row.get("duration_sec", 0) or 0
                )
            except ValueError:
                pass
    return stats
...
def stratified_select(
    speaker_gender: dict, fraction: float, seed: int
) -> dict:
    """
    ...
    for gender, speakers in by_gender.items():
        speakers_sorted = sorted(
            speakers
        )  # sort first for deterministic shuffling
...
def print_summary(
    ...
    for gender in total_counts:
        ...
        print(
            f"  {gender}: {orig_pct:.1f}% of full pool -> {sel_pct:.1f}% of"
            f" selected ({sel_n}/{total_n} speakers selected)"
        )
    ...
    if clip_stats:
        ...
        for spk, gender in selected.items():
            clip_counts_by_gender[gender] += clip_stats.get(spk, {}).get(
                "clip_count", 0
            )
            duration_by_gender[gender] += clip_stats.get(spk, {}).get(
                "duration_sec", 0.0
            )
        ...
        print(
            "\nResulting CLIP-level ratio -- SELECTED group (engine A, e.g."
            " MMS):"
        )
        ...
        print(
            f"  TOTAL: {total_selected_clips} clips,"
            f" {total_selected_duration_hr:.2f} hours"
        )

        # flag if clip-level ratio has drifted meaningfully from
        # speaker-level ratio
        for gender in total_counts:
            speaker_pct = (
                100 * selected_counts.get(gender, 0) / len(selected)
                if selected
                else 0
            )
            clip_pct = (
                100
                * clip_counts_by_gender.get(gender, 0)
                / total_selected_clips
                if total_selected_clips
                else 0
            )
            drift = abs(speaker_pct - clip_pct)
            if drift > 5:
                print(
                    f"\n⚠ {gender}: speaker ratio ({speaker_pct:.1f}%) and"
                    f" clip ratio ({clip_pct:.1f}%) differ by {drift:.1f}"
                    " points -- some speakers in this group likely have"
                    " disproportionately more/fewer clips."
                )


def print_complement_summary(
    speaker_gender: dict, complement: dict, clip_stats: dict = None
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Stratified speaker selection for MMS processing"
    )
    parser.add_argument(
        "--manifest", required=True, help="Path to manifest_bonafide.csv"
    )
    parser.add_argument(
        "--fraction",
        type=float,
    ...
    parser.add_argument(
        "--out", required=True, help="Output CSV of selected speakers"
    )
    parser.add_argument(
        "--out-complement",
        default=None,
        help=(
            "Optional: also write the unselected speakers "
    ...
    if not speaker_gender:
        print(
            "ERROR: no speakers with gender info found in manifest.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print_summary(
        speaker_gender, selected, args.fraction, args.seed, clip_stats
    )
    print(f"\nSelected speaker list written to: {Path(args.out).resolve()}")

    if args.out_complement:
        complement = write_complement(
            speaker_gender, selected, Path(args.out_complement)
        )
        print_complement_summary(speaker_gender, complement, clip_stats)
        print(
            "\nComplement speaker list written to:"
            f" {Path(args.out_complement).resolve()}"
        )
